...
const Home = () => {
  ...
  // Filter activities for the selected date that are in-progress
  const activitiesForSelectedDate = activities.filter(activity => {
    const activityDate = new Date(activity.startDate);
    return isSameDay(activityDate, selectedDate) && activity.status === 'in-progress';
  });
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Visualize e gerencie as atividades em andamento para cada dia.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-semibold">
          Atividades em andamento para {format(selectedDate, "dd 'de' MMMM", { locale: pt })}
        </h2>
        ...
        {loading ? (
              ...
              >
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="flex items-center gap-1">
                    ...
                    {activity.endDate && ` - ${format(new Date(activity.endDate), 'HH:mm')}`}
                  </CardDescription>
                </CardHeader>
                <CardContent>
          ...
          <div className="flex flex-col items-center justify-center py-10 border rounded-lg bg-muted/30">
            <p className="text-muted-foreground">Nenhuma atividade em andamento para este dia.</p>
            <Button 
              variant="link" 
